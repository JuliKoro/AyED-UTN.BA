...
Nodo* apareo(Nodo* &p, Nodo* &q) {
	Nodo * fusion = NULL;
	while (p != NULL && q != NULL) {
		if (p -> info.id <= q -> info.id) {
			agregarNodo(fusion, p -> info);
			p = p -> sig;
		} else {
			agregarNodo(fusion, q -> info);
			q = q -> sig;
		}
	}
	while (p != NULL) {
...
int main(){
    Nodo* p = NULL; //Creo Lista A
    Nodo* q = NULL; //Creo Lista D
    Nodo* nodito = NULL; //Para la busqueda
    Nodo* fusion = NULL; //Para el apareo
    int menu = -1, idBusq;
    while(menu != 0){
        ...
        switch(menu){
            ...
            case 4:
                ...
                break;
            case 5:
                agregarNodo(q, crearCanal(28, "http://noticiasdelmundo.tv/canal28", 55));
                agregarNodo(q, crearCanal(8, "https://livetv.com/channel8", 23));
                agregarNodo(q, crearCanal(51, "http://sports.tv/football", 89));
                agregarNodo(q, crearCanal(112, "https://livenews.tv/newzeland", 3));
                agregarNodo(q, crearCanal(54, "http://maspelis.com/comedia", 0));
                agregarNodo(q, crearCanal(99, "https://technews.tv/mobileworld", 56));
                ordenar(q);
                ordenar(p);
                eliminarCanales(p);
                eliminarCanales(q);
